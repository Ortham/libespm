...
namespace parser{
	...
	inline string GameFile::getFileHeader(){
		return header;
	}
	inline string GameFile::getFileName(){
		return fileName;
	}
	inline void GameFile::setFileHeader(ifstream &file){
		getline(file, header);
	}
	inline void GameFile::setFileName(string name){
		fileName = name;
	}
	/*END OF LINE*/
}
