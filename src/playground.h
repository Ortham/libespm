...
//Also test for Oblivion, FO3 and FNV plugins, needs to work for those too.
...
namespace espm {
    ...
    struct Record {
        ...
        uint32_t Read(std::ifstream &input, const Settings& settings) {
            ...
            uint32_t count = 0;
            if (flags & settings.comp_flag) {
                
                unsigned int decompSize;
                input.read(reinterpret_cast<char*>(&decompSize), 4);

                char * decompData = new char[decompSize];
                char * compData = new char[dataSize-4];

                input.read(compData, dataSize-4);

                int ret = uncompress((Bytef*)compData, (uLongf*)&decompData, (Bytef*)compData, dataSize-4);

                while (count < decompSize) {
                    Field field;
                    count += field.Read(decompData, settings);
                ...
                while (count < dataSize) {
                    Field field;
                    count += field.Read(input, settings);
                    fields.push_back(field);
                }
            }
  ...
  //                  std::cout << std::string(record.type, 4) << '\t' << std::hex << record.id << std::dec << std::endl;
                }
            }
          ...
          /*  uint32_t pos = input.tellg();
            ...
            std::cout << int32_t(count - groupSize) << std::endl
                      << int32_t(pos2 - pos) << std::endl
                      << input.eof() << std::endl;*/
