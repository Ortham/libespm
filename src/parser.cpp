...
bool parser::isBSA(ifstream &file){
...
bool parser::isVar(string data){
	return (data[0] == ':' && data[1] == ':');
}
int parser::countLeading(string line){
	int count = 0;
...
void parser::eraseLeading(string &line){
	int leading = countLeading(line);
	line.erase(0, leading);
}
void parser::eraseTrailing(string &line){
	if(line.size() >= 4){
